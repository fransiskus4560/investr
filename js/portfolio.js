...
function setActiveMarketPF(market) {
  PF.market = market;
  Store.set('investr-market', market);
  document.querySelectorAll('.market-pill').forEach(b => b.classList.toggle('active', b.dataset.market===market));
  renderPortfolio();
}

/* ── Render ─────────────────────────────────────────────── */
/* Current prices (sample until live data in step 5).
   Keyed by ticker. Used to value derived holdings.          */
const CURRENT_PRICE = {
  BBRI:5125, BBCA:9700, TLKM:3220, PGAS:1665, ASII:5175, BMRI:6450,
  UNVR:2080, ICBP:11200, KLBF:1520, GOTO:56, ANTM:1580, INDF:6700,
};
...
function renderPortfolio() {
  ...
  // ── Derive holdings from transactions (source of truth) ──
  // Transactions are IDR/IDX-based in v1 sample. For IDX, build
  // live holdings from the journal. Other markets use sample data
  // until their transactions exist.
  let rows;
  let derivedRealized = 0;
  const useDerived = (PF.market === 'IDX') && window.HoldingsEngine;

  if (useDerived) {
    const txs = HoldingsEngine.loadTransactions();
    const derived = HoldingsEngine.deriveHoldings(txs).filter(h => h.qty > 0);
    derivedRealized = HoldingsEngine.deriveHoldings(txs).reduce((s,h)=>s+(h.realizedPnl||0),0);
    if (derived.length) {
      rows = derived.map(h => {
        const now = CURRENT_PRICE[h.ticker] || h.avgBuy;
        const costVal = h.totalCost;
        const nowVal  = now * h.qty;
        const pnl     = nowVal - costVal;
        const pnlPct  = costVal ? (pnl / costVal * 100) : 0;
        return {
          ticker:h.ticker, name:h.name, acc:h.acc, accColor:h.accColor,
          intent:h.intent, lots:h.qty, buy:h.avgBuy, now,
          shares:h.qty, costVal, nowVal, pnl, pnlPct,
        };
      });
    }
  }

  // fallback: static sample (non-IDX markets, or no transactions yet)
  if (!rows) {
...
function bindPFEvents() {
  ...
  // re-render when cloud data arrives after login
  document.addEventListener('investr-cloud-synced', () => renderPortfolio());
}
